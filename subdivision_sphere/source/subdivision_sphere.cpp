...
void init(){
  ...
  mysphere.subdivide(4);

  glBindVertexArray( vao );
  glBindBuffer( GL_ARRAY_BUFFER, buffer );
  unsigned int vertices_bytes = mysphere.vertices.size()*sizeof(vec4);
  unsigned int normals_bytes  = mysphere.normals.size()*sizeof(vec3);
    
  glBufferData( GL_ARRAY_BUFFER, vertices_bytes + normals_bytes, NULL, GL_STATIC_DRAW );
  unsigned int offset = 0;
  glBufferSubData( GL_ARRAY_BUFFER, offset, vertices_bytes, &mysphere.vertices[0] );
  offset += vertices_bytes;
  glBufferSubData( GL_ARRAY_BUFFER, offset, normals_bytes,  &mysphere.normals[0] );
    
  glEnableVertexAttribArray( vNormal );
  glEnableVertexAttribArray( vPosition );

  glVertexAttribPointer( vPosition, 4, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(0) );
  glVertexAttribPointer( vNormal, 3, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(mysphere.vertices.size()*sizeof(vec4)) );
...
int main(void){
  ...
  while (!glfwWindowShouldClose(window)){
    ...
    glDrawArrays( GL_TRIANGLES, 0, mysphere.vertices.size() );
    // ====== End: Draw ======
